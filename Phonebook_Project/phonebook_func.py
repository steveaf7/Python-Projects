...
def create_db(self):
    ...
    with conn:
        ...
        # you must commit() to sace changes and close the database connection
        conn.commit()
    conn.close()
    first_run(self)
...
def onSelect(self,event):
    ...
    varList = event.widget #event.widget is whatever is triggering the event
    ...
    with conn:
        ...
        #This returns a tuple and we can slice it into 4 parts sing data[] during the iteration
        for data in varBody:
...
def addToList(self):
    ...
    var_fname = var_fname.strip() # This remove any blank spaces before and after the user's entry
    var_lname = var_lname.strip() # This will ensure that the first character in each word is capitalized
    var_fname = var_fname.title()
    var_lname = var_lname.title()
    var_fullname = ("{} {}".format(var_fname,var_lname)) # combine our normalied names into a fullname
    print("var_fullname: {}".format(var_fullname)) #this will print to the console, only for development 
    var_phone = self.txt_phone.get().strip()
    var_email = self.txt_email.get().strip()
##    if not "@" or not "." in var_email:
##        print("Incorrect email format!!!")
    #enforce the user to provide text in each box
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0):
        conn = sqlite3.connect('phonebook.db')
        with conn:
            cursor = conn.cursor()
            #check the database for fullname in case it already exists
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_phonebook WHERE col_fullname = '{}'""".format(var_fullname))
            count = cursor.fetchone()[0]
            chkName = count
            if chkName == 0: #If this is o then there is no existence of the fullname and we can add new data
                ...
                onClear(self) #call the function to clear all of thetextboxes
            else:
                messagebox.showerror("Name Error","'{}' already exists in the database! Please choose a different name.".format(var_fullname))
                onClear(self) # call the function to clear the textboxes
        conn.commit()
        conn.close()
    else:
...
def onDeleted(self):
    #clear the textbox in these textboxes
    self.txt_fname.delete(0,END)
    self.txt_lname.delete(0,END)
    self.txt_phone.delete(0,END)    
    self.txt_email.delete(0,END)
